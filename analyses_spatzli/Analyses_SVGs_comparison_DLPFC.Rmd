...
znorm <- t(apply(logcounts(spe_sig), 1, scale, center = TRUE, scale = TRUE))
dim(znorm)
znorm[1:3, 1:3]
...
rownames(df_centers) <- colnames(spe_sig)
colnames(df_centers) <- paste0("profile", seq_len(k))
df_centers <- cbind(df_centers, spatialCoords(spe_sig))
